...
typedef enum{
	...
	STANDBY_MODE = 0b0101,	//Device not reporting time, only ID and sensor status. Time bytes will be all 0xFF
	RACE_MODE = 0b0110,		// Device on full report mode. Interruptions and time reporting activated
} BEACONMODE_TypeDef;


typedef enum{

	BEACON1 = 0b0101,	//Device not reporting time, only ID and sensor status. Time bytes will be all 0xFF
	BEACON2 = 0b0110,		// Device on full report mode. Interruptions and time reporting activated
	BEACON3 = 0b1010,
	BEACON4 = 0b1100,
	HOST    = 0b1000,
} BEACONID_TypeDef;
