...
const uint8_t cc1100_GFSK_1_2_kb[] = {
                    ...
                    0x01,  // IOCFG0        GDO0_Pin Output Pin Configuration
...
// Rf settings for CC1101 Joao Carvalho

const uint8_t cc1100_OOK_5_kb[25][2] = {
...
void rf_set_modulation_mode(MODULATION_TypeDef mode){
    ...
    if(cfg_reg == cc1100_OOK_5_kb){

    	uint8_t len = sizeof(cc1100_OOK_5_kb)/sizeof(cc1100_OOK_5_kb[0]);
    	uint8_t which_reg = 0xFF;
    	for (uint8_t i = 0x00; i < len; i++)
    	{
    		rf_write_register(cc1100_OOK_5_kb[i][0], cc1100_OOK_5_kb[i][1]);
    		which_reg = rf_read_register(cc1100_OOK_5_kb[i][0]);

    	}

    }
    else rf_write_data(WRITE_BURST(0), cfg_reg, CFG_REGISTER);                            //writes all 47 config register
...
void rf_receive(){
	...
	rf_sidle();
	rf_write_register(IOCFG0, 0x07);
	rf_read_register(RXFIFO);    	//Necessary to clear GD0 pin due to IOCFG0 = 0x07 clearing conditions.
...
FRAMESTATUS_TypeDef send_frame(uint8_t* frame, int len){
	...
	if(len>FIXED_LENGTH_LIMIT){ //Use infinite packet length mode
		...
		if(len>FIFO_SIZE){ //Use variable packet length mode
			...
			len_sent+=FIFO_SIZE;
			if(!_keep_transmiting_data(&frame[len_sent], len-len_sent)) return TIMEOUT;
		}else{ //If len <= FIFO_SIZE, the FIFO needs to be filled once -changed by JOAO

			Len = sprintf(Message, "%c%s", len, frame);
			//rf_write_data(TXFIFO, frame, len);
			rf_write_data(TXFIFO, Message, Len);
			rf_write_strobe(STX);
...
FRAMESTATUS_TypeDef send_frame_fixdlen(uint8_t* Payload, uint8_t len){


		uint8_t data_in_fifo = 0x00;

		rf_sidle(); //Sets RF to idle state
		rf_write_register(PKTCTRL0, 0x04); // Ensures CC1101 is on fixed packet len w/ CRC calculation ON
		rf_write_register(PKTLEN, len); //Ensures pkt len is according to specified by function call. Changes here must be applied to emmiter.
		...
		//we need to fill the buffer before activating TX mode, or the chip will get into tx underflow state.
		rf_write_data(TXFIFO, Payload, len); //fill the buffer completely
		rf_write_strobe(STX); //Start transmision

		while(rf_read_register(MARCSTATE) != IDLE ){} //CC1101 is programmed to return to IDLE after TX
...
FRAMESTATUS_TypeDef receive_frame_fixdlen(uint8_t* frame_buffer, uint8_t len,  uint8_t* lqi, uint8_t* rssi)
{




	uint8_t CRC_bit = 0b10000000;
	uint8_t status = 0;
	...
	status = (status & CRC_bit) >> 7;


	rf_read_data(RXFIFO, frame_buffer, len);
	...
	if (status)
	{
		...
		My_LQI = rf_read_register(LQI);
		lqi = My_LQI & 0b01111111;
		rssi = rf_read_register(RSSI);


		rf_write_strobe(SIDLE);
		rf_write_strobe(SFRX);
		rf_receive();
...
FRAMESTATUS_TypeDef receive_frame(uint8_t* frame_buffer, uint16_t* len, uint8_t data_len_loc, uint8_t data_len_size, uint8_t* lqi, uint8_t* rssi){
	...
	*len = 0;
	...
	//configure interruption. Trigger when RX Buffer above threshold. - Alterado por Joao
