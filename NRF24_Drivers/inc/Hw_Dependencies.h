...
#ifndef HW_DEPENDENCIES_H_
#define HW_DEPENDENCIES_H_
...
static inline uint8_t nRF24_LL_RW(uint8_t data) {
    ...
    Status = HAL_SPI_TransmitReceive(&hspi1,&data,&result,1,2000);

    if(Status == HAL_BUSY) {
    	for(i = 0x00; i <= Max_Retries; i++){
    		Status = HAL_SPI_TransmitReceive(&hspi1,&data,&result,1,2000);
    		if(Status == HAL_OK) return result;
    	}
        Error_Handler();
    }
    return result;
